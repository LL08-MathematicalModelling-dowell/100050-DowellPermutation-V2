...
function Calculator() {
  const [keyboardChars] = React.useState([
    "A",
    "B",
    "C",
    "D",
  ...
  const totalPermutation = useSelector(
    (state) => state.userId.permutationResult
  );
  ...
  return (
    <div className="calculator">
      <div className="permutations">
        ...
        />
        <p>
          Total amount in a set 'n' = <span>{n}</span>
        </p>
        <p>
          Amount in each subset 'r' = <span>{r}</span>
        </p>
        <p>
          Total number of permutations are = <span>{totalPermutation}</span>
        </p>
      </div>
      ...
      <div className="boxes">
        {keyboardChars.map((keyboardChar) => {
          return <Box key={keyboardChar} keyboardChar={keyboardChar} />;
        })}
      </div>
      <div className="nSet">
        <h3 className="hint">Select {n} variables from the keyboard above</h3>
      ...
      <div className="rSet">
        {rArrayD.length !== 0 && (
          <h3 className="hint">Select {r} variables from the keyboard above</h3>
        )}
        <div className="savedB">
          {SData.length !== 0
            ? SData.map((sd) => {
                return (
                  <div key={sd} className="savedDiv">
                    <p>Step-{SData.indexOf(sd) + 1}</p>
                    <SavedBox key={sd} sData={sd} />
                  </div>
                );
              })
            : ""}
        ...
        {rArrayD.length !== 0 ? (
          isClicked ? (
            <p className="hint">Click on the character to save it</p>
          ) : SData.length === Number(r) ? (
            <div className="final">
              <p className="">Final permutation</p>
              <SavedBox sData={SData[r - 1]} />
            </div>
          ) : (
            <p className="hint2">Now select any variable from above</p>
          )
        ) : (
          ""
        )}
